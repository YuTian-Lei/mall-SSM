...
function init() {
    ...
    initDataGrid();
    // 异步加载初始化Ecahrt
    $.post('spotData',{years:years,region:region,screeningCount:screeningCount},(response)=>{
        $.modal.closeLoading()
        var data = response.result.spotCheckResult;
...
function initDataGrid() {
    ...
    $table.bootstrapTable({
        ...
        columns: [
            ...
            {title: "应复查人数", align: "center", field: 'allCount'},
            {title: "实复查人数", align: "center", field: 'count'},
            {title: "未复查人数", align: "center", field: 'unCount'},
        ],
...
function initScreenEcharts(data) {
    var option = {
        ...
        tooltip: {
            formatter: function () {
                ...
                var percentage = ((s.indexOf("未") != -1) ? "未复查率：" : "实复查率：") + this.point.percentage.toFixed(2) + ' %';
                return s + percentage;
            },
            shared: true
        },
    ...
    // 初始化
    chart = Highcharts.chart('regionSpotEcharts', option);
}
