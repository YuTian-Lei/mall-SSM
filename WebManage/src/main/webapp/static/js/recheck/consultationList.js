...
function initDataGrid() {
	$table.bootstrapTable({
		...
		columns : [
			[
				...
				},
				{
					...
					colspan : 1,
					rowspan : 2,
					formatter : consultationTypeOperateFormatter,
					"class" : 'W120'
				},
				{
					title : "操作",
		...
		url : '/admin/recheck/consultationlist',
...
window.consultationOperateEvents = {
	'click .toRecheck' : function(e, value, row, index) {
		window.location.href = '/admin/recheck/toRecheck?recheckId=' + row.recheckInfoId;
	},
	'click .detail' : function(e, value, row, index) {
		window.location.href = "/admin/recheck/toAddOrUpd?id=" + row.recheckInfoId + "&type=2";
	},
};
...
function consultationOperateFormatter(value, row, index) {
	return [
		...
		row.status == 1 ? '<a class="toRecheck btn btn-warning btn-xs" href="javascript:void(0);" >' :
		'<a class="btn btn-default btn-xs" href="javascript:void(0);" disabled="diaabled">',
...
function consultationTypeOperateFormatter(value, row, index) {
	return [
		row.type == 3 ?  '<i class="fa" style="text-align: center;"></i>申请转诊' : row.type == 2 ? '<i class="fa" style="text-align: center;"></i>申请会诊' : row.type == 1 ? '<i class="fa" style="text-align: center;"></i>转诊': '<i class="fa" style="text-align: center;"></i>会诊',
	].join('');
}
...
function quShiTuFormatter(value, row, index) {
	return [
		'<a class="toQuShiTuLook btn btn-warning btn-xs" href="javascript:void(0);" >',
		'<i class="fa fa-search"></i>查看',
		'</a>  ',
...
window.quShiTuEvents = {
		'click .toQuShiTuLook' : function(e, value, row, index) {
...
window.consultationDetailEvents = {
	'click .look' : function(e, value, row, index) {
		window.location.href = "/admin/recheck/toConsultationDetail?hospitalId=" + row.hospitalId + "&consultationId=" + row.id;
	},
};
