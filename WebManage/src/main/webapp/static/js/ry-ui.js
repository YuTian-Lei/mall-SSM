...
(function ($) {
    $.extend({
        ...
        modal: {
            ...
            // 弹出层指定宽度
            openDetailById: function (title, url, width, height, callback) {
                //如果是移动端，就使用自适应大小弹窗
                if (navigator.userAgent.match(/(iPhone|iPod|Android|ios)/i)) {
                    width = 'auto';
                    height = 'auto';
            ...
            // 弹出层全屏
            openDetail: function (title, url, width, height) {
                //如果是移动端，就使用自适应大小弹窗
                if (navigator.userAgent.match(/(iPhone|iPod|Android|ios)/i)) {
                    width = 'auto';
                    height = 'auto';
            ...
            ajaxDownload: function (requestMethod, url, params) {
                if ($.common.isEmpty(url)) {
                    url = "/404.html";
                }
                if ($.common.isEmpty(requestMethod)) {
                    requestMethod = "GET";
                }
                $.modal.ajaxDownload(requestMethod, url, params, ajaxMsg.LOADING_MSG, ajaxMsg.WARNING_MSG, ajaxMsg.ERROR_MSG, ajaxMsg.SUCCESS_MSG)
            },
            ajaxDownload: function (requestMethod, url, params, loadingMsg) {
                if ($.common.isEmpty(url)) {
                    url = "/404.html";
                }
                if ($.common.isEmpty(requestMethod)) {
                ...
                if ($.common.isEmpty(loadingMsg)) {
                    loadingMsg = ajaxMsg.LOADING_MSG;
                }
                $.modal.ajaxDownload(requestMethod, url, params, loadingMsg, ajaxMsg.WARNING_MSG, ajaxMsg.ERROR_MSG, ajaxMsg.SUCCESS_MSG)
            },
            ajaxDownload: function (requestMethod, url, params, loadingMsg, warnMsg) {
                if ($.common.isEmpty(url)) {
                    url = "/404.html";
                }
                if ($.common.isEmpty(requestMethod)) {
                ...
                if ($.common.isEmpty(warnMsg)) {
                    warnMsg = ajaxMsg.WARNING_MSG;
                }
                $.modal.ajaxDownload(requestMethod, url, params, loadingMsg, warnMsg, ajaxMsg.ERROR_MSG, ajaxMsg.SUCCESS_MSG)
            },
            ajaxDownload: function (requestMethod, url, params, loadingMsg, warnMsg, errorMsg) {
                if ($.common.isEmpty(url)) {
                    url = "/404.html";
                }
                if ($.common.isEmpty(requestMethod)) {
                ...
                if ($.common.isEmpty(errorMsg)) {
                    errorMsg = ajaxMsg.ERROR_MSG;
                }
                if ($.common.isEmpty(warnMsg)) {
                    warnMsg = ajaxMsg.WARNING_MSG;
                }
                $.modal.ajaxDownload(requestMethod, url, params, loadingMsg, warnMsg, errorMsg, ajaxMsg.SUCCESS_MSG)
            },
            /**
             *  异步下载
             ...
             * @param warningMsg
             */
            ajaxDownload: function (requestMethod, url, params, loadingMsg, warnMsg, errorMsg, successMsg) {
                if ($.common.isEmpty(requestMethod)) {
                    requestMethod = "GET";
                }
                if ($.common.isEmpty(url)) {
                ...
                if ($.common.isEmpty(successMsg)) {
                    successMsg = ajaxMsg.SUCCESS_MSG;
                }
                if ($.common.isEmpty(warnMsg)) {
                    warnMsg = ajaxMsg.WARNING_MSG;
                    backgroundColor:"#fff";
                ...
                xhr.open(requestMethod, url + "?" + params, true);    // 也可以使用POST方式，根据接口
                xhr.responseType = "blob";
                xhr.onload = function () {
                    var content = this.response;
                    if (this.status === 200) {
                        var contentLength = xhr.getResponseHeader("content-length");
                        if (contentLength == 0 || !xhr.getResponseHeader("Content-disposition")) {
                            console.log('%c Ajax-Download Waring : Response Content Is Null', 'color:#FFB023;')
                            $.modal.msgWarning(warnMsg);
                            return;
                        }
                        $.modal.msgSuccess(successMsg);
                        var aTag = document.createElement('a');
                        var blob = new Blob([content]);
                        var headerName = xhr.getResponseHeader("Content-disposition").substring(21);
                        // 编码格式化
                        var fileName = decodeURI(escape(headerName.substring(0, headerName.length - 1)));
                        console.log("%c Ajax-Download Success : " + fileName, 'color:#14C7B9;')
                        aTag.download = decodeURIComponent(fileName);
                        aTag.href = URL.createObjectURL(blob);
                        aTag.click();
                        URL.revokeObjectURL(blob);
                    } else {
                        console.log("%c Ajax-Download Error : " + xhr.getAllResponseHeaders(), 'color:#EE0A0A;')
             ...
             * @param warningMsg
             */
            ajaxUploadAndDownload: function (requestMethod, url, params, loadingMsg, warnMsg, errorMsg, successMsg) {
                if ($.common.isEmpty(requestMethod)) {
                    requestMethod = "GET";
                }
                if ($.common.isEmpty(url)) {
                ...
                $.modal.loading(loadingMsg)
                var xhr = new XMLHttpRequest();
                xhr.open(requestMethod, url , true);    // 也可以使用POST方式，根据接口
                xhr.responseType = "blob";
                xhr.onload = function () {
                    var content = this.response;
                    if (this.status === 200) {
