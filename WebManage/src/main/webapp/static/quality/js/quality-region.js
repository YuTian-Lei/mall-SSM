 ...
 * @param regionId
 */
function initTableData(regionId) {
    ...
    // 异步加载
    $.post($url + '/visual/quality/regionScreeningQualityStat', params, (response) => {
        ...
        if (null != response.result.data) {
            response.result.data.forEach((item) => {
                ...
                item_1 = '       <li>地区：<span class="">' + regionName + '</span></li>\n' +
 ...
 * @param datas
 */
function initScreenRegion(datas) {
    ...
    datas.forEach((item, index) => {
        ...
        if (index == 0) {
            firstId = region;
            firstName = name;
            team_list_str += '<li class="active" value="' + region + '" style="color: ' + selColor + '" titles="' + value + '">' + name + '</li>';
        } else {
            team_list_str += '<li value="' + region + '" style="color: ' + fontColor + '" titles="' + value + '">' + name + '</li>';
        }
    })
    // 填充地区
    $('.container .right .team-box ul').html(team_list_str);

    $('.header .right .big').html('深圳市' + firstName);

    // 根据地区ID加载表格数据
    initTableData(firstId);
    ...
    $('.container .right .team-box ul li').on('click', function () {
        ...
        $('.header .right .big').html('深圳市' + regionName);
...
function main(result) {
    ...
    var data = result.result.regionData;

    // 滚动条每次重新加载需销毁
    $('.header .right .team-box ul').mCustomScrollbar('destroy')
    //
    // 初始化Echart
    initQualityEchart(data);
    // 地区
    initScreenRegion(data)
...
function init() {
    ...
    // 异步加载
    $.post($url + '/visual/quality/regionScreeningQualityEchartData', getParams(), (response) => {
