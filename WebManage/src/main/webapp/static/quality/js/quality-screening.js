$(function () {
    ...
    // 左侧地区标题
    $('.left .big').html('深圳市' + getRegionName())
    $('#region').change(() => {
        // 左侧标题块展示
        $('.left .big').html('深圳市' + getRegionName())
    })
...
function initTableData(teamId) {
    ...
    // 异步加载
    $.post($url + '/visual/quality/screeningQualityStat', params, (response) => {
        ...
        if (null != response.result.data) {
            response.result.data.forEach((item) => {
                ...
                item_1 = '       <li>队长：<span class="">' + groupLeader + '</span></li>\n' +
                    ...
                    '            <li>筛查学校：<br /><span class="">' + schoolName + '</span></li>'
...
function initScreenTeam(datas) {
    ...
    // 填充筛查队
    $('.header .right .team-box ul').html(team_list_str)

    // 根据筛查队ID加载表格数据
    initTableData(first);

    // 切换队伍样式修改
    $('.header .right .team-box ul li').on('click', function () {
        $('.header .right .team-box ul li').removeClass('active')
    ...
    // 队伍滚动条
    $('.header .right .team-box ul').mCustomScrollbar({
        autoHideScrollbar: false,
        scrollbarPosition: "outside",
        axis: "x",
...
function init() {
    ...
    // 异步加载
    $.post($url + '/visual/quality/screeningQualityEchartData', params, (response) => {
